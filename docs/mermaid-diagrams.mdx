...
**appear one by one** instead of lighting up. Once the last is in, the
exchange holds for a moment, then the round starts over. The PDF is
unaffected either way — it plays no animation and prints every message —
and so is a reader whose system asks for reduced motion.
