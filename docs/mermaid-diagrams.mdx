...
reader presses **Pause** under any moving diagram, which shows it at rest
(the same figure the PDF prints); Open and Download keep the published file.
And a build run with `DIAGRAM_MOTION=off` in its environment stills every
diagram of the site at once.
