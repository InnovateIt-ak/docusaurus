...
flowchart LR
    ...
    A e3@-.->|event| Q[Queue]
    Q e4@--> D[(Store)]
    classDef step1 animation: diagram-step 7.5s linear infinite 0s
    classDef step2 animation: diagram-step 7.5s linear infinite 1.5s
    classDef step3 animation: diagram-step 7.5s linear infinite 3s
    classDef step4 animation: diagram-step 7.5s linear infinite 4.5s
    classDef step5 animation: diagram-step 7.5s linear infinite 6s
    class U step1
    class e1,W step2
    class e2,A step3
    class e3,Q step4
...
The highlight lasts a fixed tenth of the cycle, so a long chain overlaps its
steps into a wave and a short one leaves a beat between them; both read as an
order. A dotted edge given a step keeps its dashes but not its drift — the
step's animation takes the slot. Mermaid's `classDef` accepts no comma inside
a value, which is why the edge needs an `id@` and a class rather than a
`linkStyle`.
...
of rest); `autonumber` puts the step numbers on the page too.
...
sequenceDiagram
    ...
    U->>W: Sign in
    W->>+A: POST /token
    Note over A: validates the credentials
    A-->>-W: 200 token
    W->>W: store session
    W-->>U: Redirect
```
...
Notes, loops and activations sit between the steps without shifting them.
As everywhere else, nothing is hidden: the PDF shows the full exchange at
rest, and `prefers-reduced-motion` stills it.
