...
Three switches, one per audience. An author writes `%% still` in a diagram
and nothing in it moves — not even the theme's drift on dotted edges. A
reader presses **Stop** under a diagram that moves — the button appears only
there — and gets the diagram as drawn, in its own colours, the same figure
the PDF prints; **Play** starts the round again, and Open and Download keep
the published file. And a build run with `DIAGRAM_MOTION=off` in its
environment stills every diagram of the site at once.
...
Two ways. The quick one is a comment, `%% steps` (or `%% steps reveal`):
the edges then light — or appear — one by one in the order they are written,
nodes staying put, exactly as for the sequence diagram further down. The
by-hand one, used below because it also lights the **nodes**: the theme
provides the `diagram-step` animation; a `classDef` per step gives it its
delay, and `class` puts nodes and edges (by their `id@`) on that step — here,
each step lights the arrow and the node it reaches. Cycle = 1.5s ×
...
A sequence diagram has no `classDef` and no id on a message, so the by-hand
...
Or ask for the other telling: `%% steps reveal`, as below, and the messages
**appear one by one** instead of lighting up. What sits between two messages
goes with the next one — a note, an activation bar, the frame of an `alt`,
`par` or `loop` and its dividers, the autonumber — so nothing gives the story
away before its arrow. Once the last is in, the exchange holds for a moment,
then the round starts over. The PDF is unaffected either way — it plays no
animation and prints every message — and so is a reader whose system asks
for reduced motion.
...
a `par` block for good measure: eleven messages, a round of 19.5 seconds. The
...
page is the step. Because the renderer knows the count, each step's timing is
set in seconds (a highlight ramps in over 0.3s and holds to 1.2s; a reveal
fades in over 0.3s), the same on a long round as on a short one. Notes,
blocks and activations sit between the steps without shifting them. As everywhere else, nothing is hidden: the PDF shows the full
